...
from flights_pipeline.assets.raw import aviation_edge_raw, airports_raw, weather_raw_init
from flights_pipeline.utils.transform_utils import (
    enrich_with_weather,
    enrich_with_airports,
    impute_missing_actual_times,
    clean_aviation_data,
    remove_cancelled_flights,
    clean_weather,
    add_flag_columns,
    add_nearest_hour_column,
    standardize_column_names,
    rename_weather_columns,
    clean_airport_data,
...
def flights_cleaned(context, pg: PostgresResource) -> Output:
    flight_df = pd.read_sql("SELECT * FROM raw.flights_raw", pg.get_connection())
    ...
    flight_df = add_nearest_hour_column(flight_df, time_col="scheduled_departure_time", new_col="rounded_scheduled_hour")
    flight_df = rename_weather_columns(flight_df)
    flight_df = standardize_column_names(flight_df)
    
    pg.persist(flight_df, table_name="flights_cleaned", schema="stage", procedure="replace")
...
def weather_cleaned(context, pg: PostgresResource) -> Output:
    weather_df = pd.read_sql("SELECT * FROM raw.weather_raw", pg.get_connection())
    ...
    weather_df = standardize_time_columns(weather_df, time_columns=["observation_time"], timezone="Europe/Paris")

    pg.persist(weather_df, table_name="weather_cleaned", schema="stage", procedure="replace")
...
def flights_enriched_weather(context, pg: PostgresResource, flights_cleaned: pd.DataFrame, weather_cleaned: pd.DataFrame) -> Output:
    enriched_df = enrich_with_weather(flights_cleaned, weather_cleaned)

    enriched_df.drop(columns=["created_at", "source_timestamp", "timestamp", "iata"], errors="ignore", inplace=True)
...
def airports_cleaned(context, pg: PostgresResource) -> Output:
    ...
    pg.persist(df_cleaned, table_name="airports_cleaned", schema="stage", procedure="replace")
...
def flights_enriched_weather_airports(context, pg: PostgresResource, flights_enriched_weather: pd.DataFrame) -> Output:
    airports_df = pd.read_sql("SELECT * FROM raw.airports", pg.get_connection())
    enriched_df = enrich_with_airports(flights_enriched_weather, airports_df)

    pg.persist(enriched_df, table_name="flights_enriched", schema="stage", procedure="replace")
