...
def standardize_column_names(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    dataframe.columns = [
        col.strip().lower().replace(' ', '_') for col in dataframe.columns
    ]
    return dataframe
...
def standardize_time_columns(
    df: pd.DataFrame, time_columns: list, timezone: str = 'Europe/Paris',
) -> pd.DataFrame:
    """
...
def clean_aviation_data(flight_df: pd.DataFrame) -> pd.DataFrame:
    ...
    flight_df = standardize_time_columns(
        flight_df, datetime_cols, timezone='Europe/Paris',
    )

    # Remove rows with future flight_date
    if 'flight_date' in flight_df.columns:
        flight_df = flight_df[
            flight_df['flight_date'].dt.date
            <= pd.Timestamp('now', tz='Europe/Paris').date()
        ]
    ...
    flight_df.drop(
        columns=['created_at', 'source_timestamp',
                 'icao', 'estimaed_arrival_icao'],
        inplace=True,
        errors='ignore',
    )
...
def impute_missing_actual_times(flight_df: pd.DataFrame) -> pd.DataFrame:
    ...
    flight_df['scheduled_departure_time'] = pd.to_datetime(
        flight_df['scheduled_departure_time'], errors='coerce',
    )
    flight_df['actual_departure_time'] = pd.to_datetime(
        flight_df['actual_departure_time'], errors='coerce',
    )
    ...
    median_durations = route_groups.apply(
        lambda g: (g['actual_departure_time'] -
                   g['scheduled_departure_time']).median(),
    ).to_dict()
...
def remove_cancelled_flights(flight_df: pd.DataFrame) -> pd.DataFrame:
    ...
    now = pd.Timestamp.now(tz='Europe/Paris')
    mask = flight_df['actual_departure_time'].isna() & (
        now - flight_df['scheduled_departure_time'] > timedelta(days=1)
    )
    cleaned_df = flight_df[~mask]
    logger.info(
        f"Removed {flight_df.shape[0] - cleaned_df.shape[0]} cancelled flights.",
    )
    return cleaned_df
...
def add_flag_columns(flight_df: pd.DataFrame) -> pd.DataFrame:
    ...
    flight_df['is_night_flight'] = flight_df[
        'scheduled_departure_time'
    ].dt.hour.between(0, 6)
    flight_df['is_morning_flight'] = flight_df[
        'scheduled_departure_time'
    ].dt.hour.between(6, 11)
    return flight_df


@safe_execute
def add_nearest_hour_column(
    df: pd.DataFrame,
    time_col: str = 'scheduled_departure_time',
    new_col: str = 'rounded_scheduled_hour',
) -> pd.DataFrame:
    """
...
def enrich_with_weather(
    flight_df: pd.DataFrame, weather_df: pd.DataFrame,
) -> pd.DataFrame:
    """
    ...
    flight_df['departure_airport_icao'] = flight_df[
        'departure_airport_icao'
    ].str.upper()
...
@safe_execute
def enrich_with_airports(
    flight_df: pd.DataFrame, airports_df: pd.DataFrame,
) -> pd.DataFrame:
    """
    ...
    dep_airports = airports_df.rename(
        columns={'iata_code': 'departure_airport_icao'})
    flight_df = pd.merge(
        flight_df,
        dep_airports,
    ...
    arr_airports = airports_df.rename(
        columns={'iata_code': 'arrival_airport_icao'})
    flight_df = pd.merge(
        flight_df,
        arr_airports,
...
def clean_airport_data(airports_df: pd.DataFrame) -> pd.DataFrame:
    ...
    airports_df = airports_df.rename(
        columns={k: v for k, v in rename_map.items() if v})
